...
//Add Comments to an parentComments
router.post('/fetchComments', auth.verifyToken, async (req, res, next) => {
  let parentCommentID = req.body.comment.commentId;
  try {
    ...
    if (!parentComment) {
      return res
        .status(400)
        .json({ errors: { body: ['Theres is no such comment'] } });
    }
    req.body.comment.author = req.user.userId;
    ...
    parentComment = await Comment.findByIdAndUpdate(
      { _id: parentCommentID },
      { $push: { nestedComments: comment.id } }
    );
    comment = await Comment.findById(comment.id).populate('author');
    return res
      .status(200)
